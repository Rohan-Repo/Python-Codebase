...
# Install these Python Packages with requirements.txt 
# run as - pip install -r requirements.txt
# or otherwise directly with pip - 
...
def process_csv_and_update_pdfs(csv_file_path, template_pdf_path):
    ...
    for index, row in df.iterrows():
        ...
        if len(name) >= 18:
            insert_name_into_pdf( template_pdf_path, output_pdf_path, name, 300, 325, font_name='Helvetica-Bold', font_size=22, font_color=(0.2, 0.85, 0.75) )  # teal color
        else:
            insert_name_into_pdf( template_pdf_path, output_pdf_path, name, 325, 325, font_name='Helvetica-Bold', font_size=25, font_color=(0.2, 0.85, 0.75) )  # teal color

csv_file_path = 'PP-Names.csv'  # Path to your CSV file

# Export this Template PDF from Canva
template_pdf_path = 'Coding-Fundamentals-Cert.pdf'  # Path to your template PDF
